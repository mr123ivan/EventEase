...
const SubcontractorCalendar = () => {
  ...
  useEffect(() => {
    const fetchUnavailableDates = async () => {
      try {
        ...
        const response = await axios.get(`https://api.eventsease.app/api/subcontractor/unavailable-dates`, {
          params: { email: email },
          headers: {
  ...
  const saveUnavailabilityToServer = async (date, reason) => {
    try {
      ...
      const response = await axios.post('https://api.eventsease.app/api/subcontractor/unavailable-dates', 
        {
          email: email,
          date: formattedDate,
          reason: reason
  ...
  const handleConfirmRemoval = async () => {
    ...
    if (id) {
      ...
      try {
        const response = await fetch(`https://api.eventsease.app/api/subcontractor/unavailable-dates/${id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`
          },
  ...
  const saveUnavailableDates = async () => {
    try {
      ...
      const savePromises = newDates.map(dateObj => {
        ...
        return fetch('https://api.eventsease.app/api/subcontractor/unavailable-dates', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
